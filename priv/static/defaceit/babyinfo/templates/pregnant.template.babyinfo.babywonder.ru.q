<script id="pregnant_tpl" type="text/x-jquery-tmpl">
<style>
  .child_name{
    display: block;
    margin: 0 auto;
...
<div class="calc_line">
<p class="calc_label">Дата последней минструации:
...
<script>
  ...
  BabyInfo.pregnant = function(){
     function getPlan(){
        var plan = null;
      BabyInfo.children.each(function(c){
          if(c.get('planingdate') && !c.get('pregnantdate')){
            plan = c;
          }
      }, this);
        return plan;
   ...
   function append(){
      var pregnantDate = [jQuery('#profile_birthday_month').val(), jQuery('#profile_birthday_day').val(),jQuery('#profile_birthday_year').val()].join('.');
   ...
   var plan = getPlan();
      ...
      child.set('name', 'Беременна');
      child.set('pregnantdate', pregnantDate);
      ...
      child.save();
            ...
            Defaceit.Window.Manager.create('Simple', {
              content: jQuery('#pregnant_tpl').html(),
